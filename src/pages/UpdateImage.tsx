...
const UpdateDogPhoto = () => {
  ...
  const UpdateImage = async (newDogData: IDog) =>
    await axios.patch(`/dogs/upload-dog-image/${id}`, newDogData, config);
      ...
      <Flex flexDirection={"row"} align={"center"} justify={"center"}>
        <form>
          ...
          <Flex direction={"row"}>
            ...
            />
            {selectedImage && (
              <Image src={selectedImage} boxSize="150px" objectFit="cover" />
            )}
          </Flex>

          {mutation.isLoading ? (
            <Button isLoading>Update Photo</Button>
          ) : (
            <Button onClick={(e) => handleSubmit(e)}>Update Photo</Button>
          )}
        </form>
      </Flex>
    </Container>
